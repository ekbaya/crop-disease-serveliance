...
package com.example.cropmonitor.cassava;
...
public class CassavaImageClassifier {

  /** Tag for the {@link Log}. */
  private static final String TAG = "MLKitDemoApp:Classifier";

  private static final String CLOUD_MODEL_NAME = "mobilenet_v1";
  private static final String LOCAL_MODEL_NAME = "cassava";
  private static final String LOCAL_MODEL_PATH = "cassava.tflite";

  /** Name of the label file stored in Assets. */
  private static final String LABEL_PATH = "cassavalabels.txt";
  ...
  private static final int DIM_IMG_SIZE_X = 150;
  private static final int DIM_IMG_SIZE_Y = 150;

  private static final float IMAGE_MEAN = 128;
  ...
  /** Initializes an {@code CassavaImageClassifier}. */
  @RequiresApi(api = Build.VERSION_CODES.KITKAT)
  CassavaImageClassifier(Activity activity) throws FirebaseMLException {
    // register cloud model source
    FirebaseCloudModelSource cloudSource = new FirebaseCloudModelSource.Builder(CLOUD_MODEL_NAME)
            .enableModelUpdates(true)
            .build();
    FirebaseModelManager.getInstance().registerCloudModelSource(cloudSource);

    // register local model source
  ...
  private synchronized List<String> printTopKLabels(byte[][] labelProbArray) {
    for (int i = 0; i < labelList.size(); ++i) {
      sortedLabels.add(
          new AbstractMap.SimpleEntry<>(labelList.get(i), (labelProbArray[0][i] - 127) / 128.0f));
